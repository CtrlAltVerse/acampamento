<?php

use cavWP\Models\Post;
use cavWP\Models\User;
use writersCampP\Utils;

$User = new User(get_current_user_id());
$Text = new Post();

?>
<?php if (is_user_logged_in()) { ?>
<?php echo $User->get('avatar', size: 32, attrs: ['class' => 'rounded']); ?>
<span class="font-medium">
   <?php echo $User->get('name'); ?>
</span>
<a href="<?php echo Utils::get_page_link('profile'); ?>"
   target="_blank">Atualizar perfil</a>
<button class="cursor-pointer"
            x-on:click="$rest.get(moon.apiUrl+'/logout?_wpnonce='+moon.nonce)" type="button">
            <i class="ri-logout-circle-r-line ri-fw"></i>
            Sair
         </button>
<?php } else { ?>
<button class="cursor-pointer" type="button" x-on:click.prevent="$store.login.method='login';login.showModal()">Entrar
   com e-mail</button>
<button class="cursor-pointer" type="button"
        x-on:click.prevent="$store.login.method='intro';login.showModal()">Facebook</button>
<button class="cursor-pointer" type="button"
        x-on:click.prevent="$store.login.method='intro';login.showModal()">Google</button>
<?php } ?>
